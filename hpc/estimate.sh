...
# Copy data to fast local storage (each node gets its own $SLURM_TMPDIR)
...
echo ""
echo "================================================================"
echo "  Training Time Estimator — $STEPS steps per track, $NUM_GPUS GPUs"
echo "================================================================"
...
run_estimate() {
    ...
    if srun python "$script" --config "$config" --max-steps "$STEPS" --skip-eval \
        ...
        # Extrapolate: steps/epoch divided by num_gpus (DDP splits data)
        ...
        TOTAL_1GPU=$(echo "scale=1; $TOTAL_HOURS * $NUM_GPUS" | bc)
        ...
        echo "  Est. total ($epochs epochs, $NUM_GPUS GPUs): ${TOTAL_HOURS} hours"
        echo "  (1-GPU equivalent: ${TOTAL_1GPU} hours)"
        echo "  -------------------------------------------------------"
    else
        END=$(date +%s)
        ELAPSED=$((END - START))
