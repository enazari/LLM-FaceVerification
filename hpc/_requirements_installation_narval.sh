...
# --- Self-submit: run `bash _requirements_installation_narval.sh` from hpc/ dir ---
...
if [ $? -ne 0 ]; then echo "ERROR: Failed to install torch"; exit 1; fi

echo "Installing torchvision to match torch 2.9.x..."
pip install --no-index 'torchvision<0.25'
if [ $? -ne 0 ]; then echo "ERROR: Failed to install torchvision"; exit 1; fi
...
if [ $? -ne 0 ]; then
    echo "WARNING: einops install failed."
fi
