...
# Trillium variant of _requirements_installation.sh (no --mem, writable --output)
# All pip installs use --no-index to avoid network retries on compute nodes.

# --- Self-submit: run `bash hpc/trillium_requirements_installation.sh` from project root ---
...
# Step 2: Set persistent venv path — /scratch is writable on compute nodes, /project is not
PERSISTENT_DIR=/scratch/enazari/face-verification-env
...
if [ $? -ne 0 ]; then
    echo "WARNING: transformers/sentencepiece install failed."
fi

# timm + einops needed by InternVL2-2B custom code
echo "Installing timm and einops..."
pip install --no-index timm einops
if [ $? -ne 0 ]; then
    echo "WARNING: timm/einops install failed."
fi

# Note: flash-attn skipped — cvmfs wheel requires torch~=2.9 which conflicts with
# torchvision 0.25 (needs torch==2.10). PyTorch 2.10 SDPA handles FlashAttention
# natively on H100, so no separate package needed.
