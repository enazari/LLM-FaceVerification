...
from src.eval.lfw import prepare_lfw, evaluate_lfw, evaluate_lfw_pairwise
from src.eval.cfp import prepare_cfp, evaluate_cfp, evaluate_cfp_pairwise
...
def main():
    ...
    pairwise = cfg["backbone"] == "internvl_pair"

    # LFW
    prepare_lfw(args.data_dir)
    eval_lfw = evaluate_lfw_pairwise if pairwise else evaluate_lfw
    acc, std, thresh = eval_lfw(
    ...
    try:
        ...
        eval_cfp = evaluate_cfp_pairwise if pairwise else evaluate_cfp
        for protocol in ("FF", "FP"):
            acc, std, thresh = eval_cfp(
